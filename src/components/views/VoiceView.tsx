...
export default function VoiceView() {
    ...
    const renderContentWithTags = (content: string) => {
        const parts = content.split(/(#[\wğşıöşüçŞİÖŞÜ]+)/g);
        return parts.map((part, index) => {
            if (part.match(/^#[\wğşıöşüçŞİÖŞÜ]+$/)) {
                return (
                    <button
                        key={index}
                        onClick={(e) => {
    ...
    const handlePost = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const extractedTags = newStatus.match(/#[\wğşıöşüçŞİÖŞÜ]+/g) || [];
    ...
    const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        const matches = textBeforeCursor.match(/#([\wğşıöşüçŞİÖŞÜ]*)$/);
    ...
    const handleDelete = async (voiceId: string) => {
        if (!confirm('Bu gönderiyi silmek istedişinize emin misiniz?')) return;
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        // Extract hashtags
        const extractedTags = Array.from(new Set(editContent.match(/#[a-zA-ZğşıöşüçŞİÖŞÜ0-9]+/g) || [])).map(t => t.toLowerCase());
                ...
                {isGlobalMode ? (
                    ...
                    >
                        ...
                        <p className="text-lg md:text-xl text-neutral-600 dark:text-neutral-300 max-w-lg mx-auto mb-10 leading-relaxed font-serif">
                            Sınırlar kalkıyor! Dünyanın dört bir yanındaki üniversite öşrencileriyle ğok yakında burada buluşacaksın.
                        </p>
