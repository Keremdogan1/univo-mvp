...
export default function VoiceView() {
    ...
    const renderContentWithTags = (content: string) => {
        const parts = content.split(/(#[\w├ğ─ş─▒├Â┼ş├╝├ç─Ş─░├û┼Ş├£]+)/g);
        return parts.map((part, index) => {
            if (part.match(/^#[\w├ğ─ş─▒├Â┼ş├╝├ç─Ş─░├û┼Ş├£]+$/)) {
                return (
                    <button
                        key={index}
                        onClick={(e) => {
    ...
    const handlePost = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (!session) return toast.error('Oturum hatas─▒');

            const extractedTags = newStatus.match(/#[\w├ğ─ş─▒├Â┼ş├╝├ç─Ş─░├û┼Ş├£]+/g) || [];
            ...
            if (res.ok) {
                ...
                toast.error(err.error || 'Bir hata olu┼ştu');
            }
        } catch (e) {
            console.error(e);
            toast.error('Payla┼ş─▒m yap─▒lamad─▒.');
        } finally {
            setIsPosting(false);
        }
    };
    ...
    const handleCommentSubmit = async (e: React.FormEvent, voiceId: string, parentId: string | null = null, customContent: string | null = null) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                toast.error('Yorum g├Ânderilemedi');
            }
        } catch (e) {
            console.error(e);
            toast.error('Hata olu┼ştu');
        } finally {
    ...
    const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        const matches = textBeforeCursor.match(/#([\w├ğ─ş─▒├Â┼ş├╝├ç─Ş─░├û┼Ş├£]*)$/);
    ...
    const handleDelete = async (voiceId: string) => {
        if (!confirm('Bu g├Ânderiyi silmek istedi─şinize emin misiniz?')) return;
        ...
        try {
            ...
            toast.success('G├Ânderi silindi.');
            fetchVoices();
        } catch (e) {
            console.error(e);
            toast.error('Silme i┼şlemi ba┼şar─▒s─▒z.');
            fetchVoices();
        }
    };
    ...
    const handleUpdate = async (e: React.FormEvent) => {
        ...
        // Extract hashtags
        const extractedTags = Array.from(new Set(editContent.match(/#[a-zA-Z├ğ─ş─▒├Â┼ş├╝├ç─Ş─░├û┼Ş├£0-9]+/g) || [])).map(t => t.toLowerCase());
        ...
        try {
            ...
            toast.success('G├Ânderi g├╝ncellendi.');
            // Refresh to update Kamp├╝ste G├╝ndem and other lists
            await fetchVoices();
        } catch (e: any) {
            console.error(e);
            toast.error(`G├╝ncelleme ba┼şar─▒s─▒z: ${e.message}`);
            fetchVoices();
        }
    };
    ...
    const formatRelativeTime = (dateString: string) => {
        ...
        if (hours < 24) return `${hours} sa ├Ânce`;
        const days = Math.floor(hours / 24);
        if (days < 7) return `${days} g├╝n ├Ânce`;
        const weeks = Math.floor(days / 7);
        if (weeks < 4) return `${weeks} hf ├Ânce`;
        const months = Math.floor(days / 30);
        if (months < 12) return `${months} ay ├Ânce`;
        const years = Math.floor(days / 365);
        return `${years} y─▒l ├Ânce`;
    };
    ...
    const handlePollVote = async (index: number) => {
        if (!user) {
            toast.error('Oy kullanmak i├ğin giri┼ş yapmal─▒s─▒n─▒z.');
            router.push('/login');
            return;
        }
        ...
        try {
            ...
            if (userVote === index) {
                ...
                setUserVote(null);
                action = 'retract';
                toast.success('Oyunuz geri al─▒nd─▒.');
            ...
            toast.error('Oylama s─▒ras─▒nda bir hata olu┼ştu.');
    ...
    useEffect(() => {
        const fetchPoll = async () => {
            try {
                ...
                console.error(e);
                setActivePoll({ question: "Hata: Anket y├╝klenemedi.", options: ["..."] });
    ...
    const fetchVoters = async () => {
        ...
        try {
            ...
            toast.error('Kat─▒l─▒mc─▒lar y├╝klenemedi');
        } finally {
        ...
        <div className="container mx-auto px-4 pt-8 pb-32 relative min-h-[100dvh]">
            ...
            <div className="border-b-4 border-black dark:border-neutral-600 pb-4 mb-8 text-center transition-colors md:static bg-neutral-50 dark:bg-[#0a0a0a] pt-4 -mt-4 -mx-4 px-4 relative min-h-[240px]">
                <div className="flex flex-col items-center justify-center gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        <div 
                            ...
                            title={isGlobalMode ? "ODT├£ Moduna Ge├ğ" : "Global Moda Ge├ğ"}
                        >
                                <div 
                                ...
                                <div className="absolute inset-0 backface-hidden rounded-full overflow-hidden border-2 border-black dark:border-neutral-400 bg-white dark:bg-black shadow-md">
                                     <img src="/odtu_logo.png" alt="ODT├£" className="w-full h-full object-cover" />
                                </div>
                                {/* Back: Global */}
                                <div 
                ...
                <div className="flex justify-between items-center text-sm font-medium border-t-2 border-black dark:border-neutral-600 pt-2 mt-4 max-w-2xl mx-auto text-neutral-600 dark:text-neutral-400 h-8">
                    ...
                    <span>{isGlobalMode ? 'D├£NYA G├£NDEM─░' : 'SERBEST K├£RS├£'}</span>
                            ...
                            <div className="lg:col-span-2 space-y-8 order-last lg:order-first">
                                ...
                                {user ? (
                                    <div className="bg-neutral-50 dark:bg-transparent p-6 border border-neutral-200 dark:border-none mb-8 rounded-sm shadow-sm dark:shadow-none relative">
                                        ...
                                        <form onSubmit={handlePost} className="relative z-50">
                                            <textarea
                                                ...
                                                placeholder="Kamp├╝s g├╝ndemi hakk─▒nda ne d├╝┼ş├╝n├╝yorsun? (#etiket kullanabilirsin)"
                                                value={newStatus}
                                            ...
                                            <div className="flex justify-between items-center border-t border-neutral-200 dark:border-neutral-800 pt-3">
                                                <label className="flex items-center gap-2 cursor-pointer group">
                                                    ...
                                                    />
                                                    <span className={`text-sm ${isAnonymous ? 'font-bold text-black dark:text-white' : 'text-neutral-500 dark:text-neutral-400'}`}>Anonim Payla┼ş</span>
                                                </label>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    >
                                                        <Send size={14} />
                                                        {isPosting ? 'Yay─▒nlan─▒yor...' : 'Yay─▒nla'}
                                                    </button>
                                                </div>
                                            </div>
                                        </form>
                                    ...
                                    <div className="bg-neutral-100 dark:bg-neutral-900 p-6 text-center border border-neutral-200 dark:border-neutral-800 mb-8">
                                        <p className="text-neutral-600 dark:text-neutral-400">Payla┼ş─▒m yapmak i├ğin <Link href="/login" className="underline font-bold text-black dark:text-white">giri┼ş yapmal─▒s─▒n</Link>.</p>
                                    </div>
                                )}
                                                    ...
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div className="flex flex-col">
                                                                ...
                                                                <span className="text-[10px] text-neutral-500 dark:text-neutral-400 font-medium">{topic.count} g├Ânderi</span>
                                                            </div>
                                                        </div>
                        ...
                        <div className="relative w-full max-w-lg bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 shadow-xl overflow-hidden animate-in zoom-in duration-200 rounded-xl">
                            ...
                            <div className="p-6 max-h-[50vh] overflow-y-auto bg-neutral-50/50 dark:bg-neutral-900/50">
                                {isLoadingVoters ? (
                                    ...
                                    <div className="text-center italic">Hen├╝z veri yok.</div>
                                ) : (
